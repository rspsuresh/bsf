...
class ReceiptController extends AbstractActionController
{
                ...
                switch($RType) {
                    ...
                    case 'getWONo':
                        $select->from( 'Proj_WORegister' )
                            ->columns( array( 'data' => 'WORegisterId', 'value' => 'WONo', 'WODate' => new Expression("FORMAT(WODate, 'dd-MM-yyyy')") ) )
                            ->where( "CostCentreId='$PostDataStr'" );
                        $statement = $sql->getSqlStringForSqlObject($select);
                        $results = $dbAdapter->query( $statement, $dbAdapter::QUERY_MODE_EXECUTE )->toArray();
                        $data = json_encode($results);
                        break;
			...
			if ($request->isPost()) {
                ...
                try {
					if($recpid == 0) {
                        ...
                        $aVNo = CommonHelper::getVoucherNo(702, date('Y/m/d'), 0, 0, $dbAdapter, "I");
                        ...
                        $connection->commit();
                        $this->redirect()->toRoute('clientbilling/receipt', array('controller' => 'receipt', 'action' => 'index'));
				...
				if ($editid != 0) {
                    ...
                    $select->from( array( "a" => "CB_ReceiptRegister" ) )
                        ->join( array( "b" => "Proj_WORegister" ), "a.WORegisterId=b.WORegisterId", array("WONo" ), $select::JOIN_INNER )
                        ->join( array( 'c' => 'WF_OperationalCostCentre' ), 'b.CostCentreId=c.CostCentreId', array( "CostCentreName", "CostCentreId" ), $select::JOIN_LEFT )
                        ->join( array( 'e' => 'Proj_ClientMaster' ), 'b.ClientId=e.ClientId', array( "ClientName", "ClientId" ), $select::JOIN_LEFT )
                                ...
                                break;
                                       ...
                                       /* $select = $sql->select();
                                        $select->from(array("a"=>'WF_OperationalCostCentre'))
                                            ->columns(array("CostCentreId","WORegisterId" => new Expression("b.WORegisterId"),"ClientId" => new Expression("b.ClientId"),"Title" => new Expression("CONCAT(b.WONo':', '" .  a.CostCentreName . "'))))
                                            ->join(array("b"=>"Proj_WORegister"), "a.WORegisterId=b.WORegisterId", array(), $select::JOIN_INNER)
                                            ->join(array("c"=>"Proj_ClientMaster"), "b.ClientId=c.ClientId", array(), $select::JOIN_INNER);
                                            //->where("DeleteFlag='0'");


                                        Select A.CostCentreId,B.WORegisterId,C.ClientId, B.WONo + ' : ' + A.CostCentreName + ' (' + C.ClientName + ')' as text  from WF_OperationalCostCentre A
                         Inner Join Proj_WORegister B on A.WORegisterId=B.WORegisterId
                         Inner Join Proj_ClientMaster C on B.ClientId=C.ClientId
                      */

                     $select21 = $sql->select();
                                 $select21->from( array( 'a' => 'WF_OperationalCostCentre' ) )
                                ->columns( array( 'CostCentreId', 'WORegisterId','data' => new Expression("c.ClientId"), 'value' => new Expression("B.WONo + ' : ' + A.CostCentreName + ' (' + C.ClientName + ')'")) )
                                ->join( array( 'b' => 'Proj_WORegister' ), 'a.WORegisterId=b.WORegisterId', array(), $select21::JOIN_INNER )
                                ->join( array( 'c' => 'Proj_ClientMaster' ), 'b.ClientId=c.ClientId ', array(), $select21:: JOIN_INNER );
                                 $statement = $sql->getSqlStringForSqlObject( $select21 );
                ...
                $aVNo = CommonHelper::getVoucherNo(702, date('Y/m/d'), 0, 0, $dbAdapter, "");
	...
	public function registerAction(){
        ...
        $select->from(array('a' => 'CB_ReceiptRegister'))
            ...
            ->join(array('d' => 'Proj_ClientMaster'), 'b.ClientId=d.ClientId', array('ClientName'), $select::JOIN_LEFT)
