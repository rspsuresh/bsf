(function($) {
    $.fn.tree_structure = function(options) {
        ...
        return this.each(function() {
			...
			$(document).on("click", '.l_menuicon_nav', function() {
                call_structure();
            });
            ...
            $(document).on("hover", '.' + class_name + ' li > div', function(event) {
                if (event.type == 'mouseenter' || event.type == 'mouseover') {
					...
					$(this).children('span.activityName').children('span.delete_action').show();
                } else {
                    $(this).children('span.highlight').hide();
					$(this).children('span.activityName').children('span.delete_action').hide();
                }
            });
            ...
            $(document).on("click", '.' + class_name + ' span.highlight', function() {
                ...
                $('.' + class_name + ' li > div.tree_div').not(".parent, .current, .children").closest('li').addClass('tnone');
                ...
                if ($('.back_btn').length == 0) {
					$('#back_btn').prepend('<span class="back_btn"><i class="fa fa-angle-double-left" aria-hidden="true"></i> <span>Back</span></span>');
                }
                call_structure();
            ...
            if (add_option) {
                $(document).on("click", '.' + class_name + ' a.add_action', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (_this.closest('div').children('form.add_data').length == 0) {
                                _this.closest('li').find('div').first().append(addquery);
                                if ((_this.closest('li').find('div').first().children('form').offset().top + _this.closest('li').find('div').first().children('form').outerHeight()) > $(window).height()) {
                                    _this.closest('li').find('div').first().children('form').css({'margin-top': -_this.closest('li').find('div').first().children('form').outerHeight()});
                                }
                                if ((_this.closest('li').find('div').first().children('form').offset().left + _this.closest('li').find('div').first().children('form').outerWidth()) > $(window).width()) {
                                    _this.closest('li').find('div').first().children('form').css({'margin-left': -_this.closest('li').find('div').first().children('form').outerWidth()});
                                }
                                _this.closest('li').find('div').first().children('form').children('input.activityName').focus();
                                _this.closest('li').find('div').first().closest('li').closest('ul').children('li').children('div').addClass('zindex');
								$(_this.closest('li').find('div').first()).popover("hide");
							}
                        }
                    });
                    $(document).on("click", "input.submit", function(event) {
                        var _addthis = $(this);
                        var actName = _addthis.closest('form').find('input.activityName');
                        var subButton = _addthis.closest('form').find('input.submit');
                        var loader = _addthis.closest('form').find('div');
                        subButton.hide();
                        loader.show();
                        if($.trim(actName.val())!="") {
                            actName.removeClass('red_bdr');
                            var ajax_add_id;
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if(data=='old'){
                                        ...
                                        ajax_add_id = data;
                                        var html_value = '<li>' + vertical_line_text + horizontal_line_text + '<div id="' + ajax_add_id + '" class="tree_div">' + highlight_text + "<span class='activityName'>" + actName.val() + "<span class='delete_action'><i class='fa fa-trash-o'></i></span></span>" + '</div></li>';
            ...
            if (edit_option) {
                $(document).on("click", '.' + class_name + ' a.edit_action', function() {
                    if ($('form.add_data').length > 0)
                        $('form.add_data').remove();
                    if ($('form.edit_data').length > 0)
                        $('form.edit_data').remove();
                    var edit_string = $(this).closest('li').find('div').first().clone();
                    ...
                    var edit_ele_id = $(this).closest('li').find('div').first().attr("id");
                    var _this = $(this);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            var editquery = data;
                            if (_this.closest('li').find('div').first().children('form.edit_data').length == 0) {
                                _this.closest('li').find('div').first().append(editquery);
                                if ((_this.closest('li').find('div').first().children('form').offset().top + _this.closest('li').find('div').first().children('form').outerHeight()) > $(window).height()) {
                                    _this.closest('li').find('div').first().children('form').css({'margin-top': -_this.closest('li').find('div').first().children('form').outerHeight()});
                                }
                                if ((_this.closest('li').find('div').first().children('form').offset().left + _this.closest('li').find('div').first().children('form').outerWidth()) > $(window).width()) {
                    ...
                    $(document).on("click", "input.edit", function(event) {
                        var _editthis = $(this);
                        var actName = _editthis.closest('form').find('input.activityName');
                        var subButton = _editthis.closest('form').find('input.submit');
                        var loader = _editthis.closest('form').find('div');
                        subButton.hide();
                        loader.show();
                        if($.trim(actName.val())!="") {
                            actName.removeClass('red_bdr');
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if(data=='old'){
                                        ...
                                        var edit_html = "";
                                        edit_html += "<span class='activityName'>" + actName.val() + "<span class='delete_action'><i class='fa fa-trash-o'></i></span></span>";
                                        if (element_target.children('b.thide') == true) {
                                            element_target.children('b.thide').nextAll().remove();
                                        } else {
                                            element_target.children('span.highlight').nextAll().remove();
                                        }
                                        ...
                                        element_target.children('span.highlight').hide();
                                        $('li > div.zindex').removeClass('zindex');
                                        call_structure();
                                        $("img.load").remove();
                                    }
            ...
            if (delete_option) {
                $(document).on("click", '.' + class_name + ' span.delete_action', function() {
                    ...
                    if (confirm_before_delete) {
						...
						var confirm_text = "Are you sure want to delete this Activity?";
                        confirm_message = confirm(confirm_text);
                    }
                    if ($(this).closest('div').attr('id') == 1 || $(this).closest('li').children('ul').length != 0) {
                        alert("You cant delete root/parent activity. Delete all child activity first");
                    } else {
                        if (confirm_message) {
